...
Citizen.CreateThread(function()
	while FastLife == nil do
		TriggerEvent('fastlife:getSharedObject', function(obj) FastLife = obj end)
		Citizen.Wait(0)
	end

	PlayerData = FastLife.GetPlayerData()
end)

RegisterNetEvent('fastlife:playerLoaded')
AddEventHandler('fastlife:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
end)

RegisterNetEvent('fastlife:setJob')
AddEventHandler('fastlife:setJob', function(job)
	PlayerData.job = job
end)

RegisterNetEvent('fastlife_ruski_areszt:aresztowany')
AddEventHandler('fastlife_ruski_areszt:aresztowany', function(target)
	Aresztowany = true
...
RegisterNetEvent('fastlife_ruski_areszt:aresztuj')
AddEventHandler('fastlife_ruski_areszt:aresztuj', function()
	local playerPed = GetPlayerPed(-1)
...
--[[Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['LEFTSHIFT']) and IsControlPressed(0, Keys['N6']) and not Aresztuje and GetGameTimer() - OstatnioAresztowany > 10 * 1000 and PlayerData.job.name == PracaPolice then	-- Mozesz tutaj zmienic przyciski
			Citizen.Wait(10)
			local closestPlayer, distance = FastLife.Game.GetClosestPlayer()
			...
			if distance ~= -1 and distance <= Config.ArrestDistance and not Aresztuje and not Aresztowany and not IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyVehicle(GetPlayerPed(closestPlayer)) then
				...
				FastLife.ShowNotification("~b~Aresztujesz Obywatela~r~ " .. GetPlayerServerId(closestPlayer) .. "")						-- Drukuje Notyfikacje
				TriggerServerEvent('fastlife_ruski_areszt:startAreszt', GetPlayerServerId(closestPlayer))									-- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
				...
				FastLife.ShowNotification("~b~Zaresztowano Obywatela ~r~ " .. GetPlayerServerId(closestPlayer) .. "")					-- Drukuje Notyfikacje
				TriggerServerEvent('fastlife_policejob:handcuff', GetPlayerServerId(closestPlayer))									-- Zakuwa Poprzez Prace fastlife_policejob, Mozna zmienic Funkcje na jaka kolwiek inna.
			end
		end
	end
end)]]--