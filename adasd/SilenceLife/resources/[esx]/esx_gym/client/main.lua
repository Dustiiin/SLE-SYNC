...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke, um ~INPUTCONTEXT~ ein ~b~Fahrrad zu mieten')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Sie haben bereits ein ~r~Fahrzeug")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ für ein ~b~Fitnessstudio~w~ Menü')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~, um deine ~G~Arme zu trainieren')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Vorbereiten der ~g~Übung~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Du musst ~r~60 Sekunden ~w~ausruhen, bevor du eine weitere Übung machst.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Du benötigst eine Mitgliedschaft, um ~r~Übungen machen zu können")
						end
					elseif training == true then
						ESX.ShowNotification("Du musst dich ausruhen...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~, um ~g~Klimmzüge zu machen')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~  ~g~ um Liegestütze zu machen')
				...
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
						...
						if membership == true then				
							...
							Citizen.Wait(30000)
							ClearPedTasksImmediately(playerPed)
							ESX.ShowNotification("Du musst dich ~r~60 sekunden ausruhen ~w~ bevor Sie eine andere Übung machen.")
						
							--TriggerServerEvent('esx_gym:trainPushups') ## Folgt...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Sie benötigen eine Mitgliedschaft um ~r~Trainieren zu können")
						end							
					elseif training == true then
						ESX.ShowNotification("Du musst dich ausruhen...")
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um folgendes zu machen: ~g~Yoga')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~, um ~g~Liegestütze zu machen')
...
function CheckTraining()
	if resting == true then
		ESX.ShowNotification("Du ruhst dich aus ...")
	...
	if resting == false then
		ESX.ShowNotification("Jetzt können Sie wieder trainieren ...")
	end
end
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Wir haben ~g~24~w~ Stunden am Tag geöffnet. Herzlich willkommen!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Gym - Tienda',
            elements = {
				{label = 'Proteinshake ($100)', value = 'protein_shake'},
				{label = 'Wasser (30 $)', value = 'water'},
				{label = 'Sport-Essen ($30)', value = 'sportlunch'},
				{label = 'Powerade ($40)', value = 'powerade'},
				{label = 'Verband ($500)', value = 'bandage'},
            }
        },
        function(data, menu)
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_ship_menu',
        {
            ...
            elements = {
				{label = 'Membership ($1800)', value = 'membership'},
            }
        },
        function(data, menu)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'bike_menu',
        {
            ...
            elements = {
				...
				{label = 'Kreuzer ($300)', value = 'cruiser'},
				{label = 'Fixierer ($329)', value = 'fixter'},
